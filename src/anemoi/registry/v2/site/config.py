...
def get_config_dir() -> Path:
    ...
    section = bootstrap.get("update-shared-config", {})
    config_dir = section.get("site_config_path")
    if not config_dir:
        raise ValueError(
            f"No update-shared-config.site_config_path in {BOOTSTRAP_PATH}\nRun --update-shared-config first."
        )
    return Path(config_dir)


def fetch_and_save_steward_config():
    """Fetch {steward_url}/config/steward and merge into steward.json."""
    from .bootstrap import BOOTSTRAP_PATH
    from .bootstrap import load_bootstrap
    ...
    url = f"{steward_url}/config/steward"
...
def fetch_and_save_shared_config():
    ...
    bootstrap = load_bootstrap()
    steward_url = bootstrap.get("steward_url")
    shared_section = bootstrap.get("update-shared-config", {})
    ...
    url = f"{steward_url}/config/shared"
